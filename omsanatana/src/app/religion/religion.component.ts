...
lastSelectedMainCategory: NzTreeNode | null = null;
...
onCategoryClick(event: NzFormatEmitEvent): void {
  ...
  if (nodeType === 'category') {
    if (this.lastSelectedMainCategory && this.lastSelectedMainCategory.key !== node.key) {
      this.collapseNodeAndChildren(this.lastSelectedMainCategory);
    }
    this.lastSelectedMainCategory = node;
  }
...
collapseNodeAndChildren(node: NzTreeNode): void {
  ...
  this.religiousservice.subcategeories(categoryId).subscribe(
    (response: any[]) => {
      ...
      if (response && response.length > 0) {
        ...
        this.notificationHelper.showSuccessNotification('No subcategories available for this category.');
      }
    },
    (err: any) => {
      console.error('Error loading subcategories:', err);
      this.notificationHelper.showSuccessNotification('Failed to load subcategories. Please try again later.');
    }
  );
}
  ...
  this.religiousservice.minisubcategeories(subcategoryId).subscribe(
    (response: any[]) => {  
      ...
      if (response && response.length > 0) {
        ...
        this.notificationHelper.showSuccessNotification('No minisubcategories available for this subcategory.');
      }
    },
    (err: any) => {
      console.error('Error loading minisubcategories:', err);
      this.notificationHelper.showSuccessNotification('Failed to load minisubcategories. Please try again later.');
    }
  );
}
...
loadSpecificCategories(node: NzTreeNode, minisubcategoryId: string | null): void {
  ...
  this.religiousservice.specificminisubcategeories(minisubcategoryId).subscribe(
    (response: any[]) => {  
      ...
      if (response && response.length > 0) {
        this.specificCategories = response.map(specCat => ({
          title: specCat.name,  
          key: specCat._id,    
          isLeaf: true, 
        ...
        node.addChildren(this.specificCategories);
        node.isExpanded = true;
      } else {
        this.notificationHelper.showSuccessNotification('No specific categories available for this minisubcategory.');
      }
    },
    (err: any) => {
      console.error('Error loading specific categories:', err);
      this.notificationHelper.showSuccessNotification('Failed to load specific categories. Please try again later.');
    }
  );
}
